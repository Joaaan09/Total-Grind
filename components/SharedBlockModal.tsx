...
interface SharedBlockModalProps {
    title: string;
    subtitle?: string;
    ...
    blockIdForNewWeek?: string;
}

export const SharedBlockModal: React.FC<SharedBlockModalProps> = ({
    title,
    subtitle,
    editor,
    onClose,
            ...
            <div className="w-full sm:max-w-2xl lg:max-w-[1600px] bg-slate-900 border border-slate-800 rounded-lg shadow-xl overflow-hidden flex flex-col max-h-[85vh] sm:max-h-[90vh]">
                ...
                <div className="flex-none flex items-start justify-between p-3 sm:p-4 border-b border-slate-800 bg-slate-950/50">
                    <div className="flex flex-col flex-1 min-w-0">
                        <h2 className="text-lg sm:text-xl font-bold text-slate-50 truncate">{title}</h2>
                        {subtitle && (
                            <p className="text-xs sm:text-sm text-brandRed-500 truncate">{subtitle}</p>
                        )}
                    </div>
                    <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-50 p-1 shrink-0 ml-2">
                        <X size={20} />
                    </button>
                </div>
                    ...
                    <div className="flex-1 overflow-auto bg-slate-950/20">
                        <div className="flex min-w-max p-6 gap-6 min-h-full">
                            ...
                            <div className="flex flex-col items-start p-3 w-[160px] shrink-0">
                                <button
                                    type="button"
                                    onClick={() => editor.addWeek(blockIdForNewWeek)}
