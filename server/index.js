  ...
  try {
    ...
    for (const lift of COMPETITION_LIFTS) {
      ...
      if (liftProgress && liftProgress.history.length > 0) {
        // Encontrar el mejor estimado y el mejor real
        let bestEstimated = 0;
        let bestActual = 0;
    ...
    // Calcular total
    const totalEstimated = (bestLifts['Comp SQ']?.estimated || 0) +
      (bestLifts['Comp BP']?.estimated || 0) +
      (bestLifts['Comp DL']?.estimated || 0);

    const totalActual = (bestLifts['Comp SQ']?.actual || 0) +
      (bestLifts['Comp BP']?.actual || 0) +
      (bestLifts['Comp DL']?.actual || 0);
...
app.get('/api/admin/blocks/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const block = await TrainingBlock.findById(id);
    if (!block) {
...
app.post('/api/admin/blocks', authMiddleware, adminMiddleware, async (req, res) => {
  try {
...
app.put('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // Prevenir cambiar el rol a admin desde aquí
    if (role === 'admin') {
      return res.status(400).json({ error: 'Cannot set admin role via API' });
    }
...
app.delete('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // Si era coach, eliminar de atletas vinculados
    if (user.role === 'coach') {
    ...
    // Si era atleta, eliminar de la lista del coach
    if (user.coachId) {
...
app.get('/api/admin/users/:id/athletes', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Buscar atletas que tienen este coach
    const athletes = await User.find({ coachId: id })
...
app.post('/api/admin/users/:id/athletes', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (!athlete || athlete.role !== 'athlete') {
      return res.status(400).json({ error: 'Invalid athlete' });
    }

    // Si el atleta ya tiene otro coach, quitar de ese coach primero
    ...
    // Asignar coach al atleta
    athlete.coachId = id;
    await athlete.save();

    // Añadir atleta a la lista del coach (si tiene el campo)
    await User.updateOne(
      { _id: id },
...
app.delete('/api/admin/users/:id/athletes/:athleteId', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // Solo permitir quitar si el atleta pertenece a este coach
    if (athlete.coachId?.toString() !== id) {
      return res.status(400).json({ error: 'Athlete does not belong to this coach' });
    }

    // Quitar coach del atleta
    athlete.coachId = undefined;
    await athlete.save();

    // Quitar atleta de la lista del coach
    await User.updateOne(
      { _id: id },
      { $pull: { athletes: athleteId } }
    );
